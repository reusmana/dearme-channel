...
func GetDataToken(param string) (models.TableApiChannel, error) {
	...
	dearme_api_channel := enums.SCHEMA_PUBLIC + "dearme_api_channel"

	err := db.Table(dearme_api_channel).
		Where(dearme_api_channel+".id =? ", param).
		Order(dearme_api_channel + ".created_date desc").
		First(&obj).Error
	return obj, err
}
...
func FindSalesOrder(param string) (models.TableSalesOrder, error) {
	...
	db := config.ConnectionPg()
	db.Exec(`set search_path='public'`)
	dearme_api_channel := enums.SCHEMA_PUBLIC + "wms_sales_order"

	err := db.Table(dearme_api_channel).
		Where(dearme_api_channel+".no_order =? ", param).
		First(&obj).Error
	return obj, err
}
...
func FindSalesOrderParam(param string, param2 string) (models.TableSalesOrder, error) {
	...
	err := db.Table(dearme_api_channel).
		Where(dearme_api_channel+".no_order =? and "+dearme_api_channel+".status_process_order = ? ", param, param2).
		First(&obj).Error
	return obj, err
}
...
func FindSalesOrderBySku(param, param2, param3 string) (models.TableSalesOrder, error) {
	...
	err := db.Table(dearme_api_channel).
		Where(dearme_api_channel+".no_order =? and "+dearme_api_channel+".status_process_order = ? and "+dearme_api_channel+".sku_no = ? ", param, param2, param3).
		First(&obj).Error
	return obj, err
}
...
func FindChannel(param string) (models.TableChannel, error) {
	...
	db := config.ConnectionPg()
	db.Exec(`set search_path='public'`)
	dearme_api_channel := enums.SCHEMA_PUBLIC + "wms_channel"

	err := db.Table(dearme_api_channel).
		Where(dearme_api_channel+".channel_name =? ", param).
		First(&obj).Error
	return obj, err
}
...
func SaveTokenShopee(obj models.TableApiChannel) error {
	...
	if err := db.Table(enums.SCHEMA_PUBLIC + "dearme_api_channel").Create(&obj).Error; err != nil {
		db.Rollback()
		return err
	}
